...
public class FeedParser {
    ...
    private List<Entry> readFeed(XmlPullParser parser)
        ...
        while (parser.next() != XmlPullParser.END_TAG) {
            ...
            // Starts by looking for the <entry> tag. This tag repeates inside of <feed>
    ...
    private Entry readEntry(XmlPullParser parser) throws IOException, XmlPullParserException {
        ...
        while (parser.next() != XmlPullParser.END_TAG) {
            // 如果等于START_TAG就会进行相应的解析操作。
            if (parser.getEventType() != XmlPullParser.START_TAG) {
                continue;
            }

            // title和link保持不变，published时间格式不一致。
            // 所以只用到title和link.
            String name = parser.getName();
            if (name.equals("id")) {
    ...
    /**
     * Skips tags that parser isn't interested in. Uses depth to handle nested tags.
     * i.e., if the next tag after a START_TAG isn't a matching END_TAG, it keep going
     * until it finds the matching END_TAG (as indicated by the value of "depth" being 0).
     */
    /**
     * 去掉多层嵌套。
     * 比如对下面的不敢兴趣，解析器就会自动忽略。
     * <h2><a ref=""></a></h2>
     */
