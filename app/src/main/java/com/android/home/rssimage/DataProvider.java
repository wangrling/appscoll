...
/**
 *
 * Defines a ContentProvider that stores URLs of 500px featured pictures
...
public class DataProvider extends ContentProvider {
    ...
    // Defines an SQLite statement that builds the 500px picture URL table.
    private static final String CREATE_PICTURE_URL_TABLE_SQL =
            "CREATE TABLE " + " " + DataProviderContract.PICTURE_URL_TABLE_NAME + " " +
            "( " +
            DataProviderContract.ROW_ID + " " +  PRIMARY_KEY_TYPE + " ," +
                    DataProviderContract.IMAGE_THUMB_URL_COLUMN + " " + TEXT_TYPE + " ," +
                    DataProviderContract.IMAGE_URL_COLUMN + " " + TEXT_TYPE + " ," +
                    DataProviderContract.IMAGE_THUMB_NAME_COLUMN + " " + TEXT_TYPE + " ," +
                    DataProviderContract.IMAGE_PICTURE_NAME_COLUMN + " " + TEXT_TYPE +
                    ")";

    // Defines an SQLite statement that builds the URL modification data table.
    private static final String CREATE_DATE_TABLE_SQL =
            "CREATE TABLE " +
    ...
    // Identifies log statements issues by this component.
    ...
    static {
        ...
        // Add a URI "match" entry that maps picture URL content URIs to a numeric code.
        sUriMatcher.addURI(
                DataProviderContract.AUTHORITY,
                DataProviderContract.PICTURE_URL_TABLE_NAME,
                IMAGE_URL_QUERY);

        // Adds a URI "match" entry that maps modification data content URIs to a numeric code.
        sUriMatcher.addURI(
                DataProviderContract.AUTHORITY,
                DataProviderContract.DATE_TABLE_NAME,
                URL_DATE_QUERY);
    ...
    private class DataProviderHelper extends SQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase db) {
            ...
            db.execSQL(CREATE_PICTURE_URL_TABLE_SQL);
        ...
        private void dropTables(SQLiteDatabase db) {
            ...
            db.execSQL("DROP TABLE IF EXISTS " + DataProviderContract.PICTURE_URL_TABLE_NAME);
    ...
    /**
     * Return the result of querying the chosen table.
    ...
    public Cursor query(@NonNull @android.support.annotation.NonNull Uri uri, @Nullable @android.support.annotation.Nullable String[] projection, @Nullable @android.support.annotation.Nullable String selection, @Nullable @android.support.annotation.Nullable String[] selectionArgs, @Nullable @android.support.annotation.Nullable String sortOrder) {
        ...
        switch (sUriMatcher.match(uri)) {
            ...
            case IMAGE_URL_QUERY:
                ...
                // Sets the ContentResolver to watch thi content URI for data changes.
    ...
    public Uri insert(@NonNull @android.support.annotation.NonNull Uri uri, @Nullable @android.support.annotation.Nullable ContentValues values) {
        ...
        switch (sUriMatcher.match(uri)) {
            ...
            // For the modification data table.
            case URL_DATE_QUERY:
     ...
     */
    @Override
    public int bulkInsert(@NonNull @android.support.annotation.NonNull Uri uri,
                          @NonNull @android.support.annotation.NonNull ContentValues[] insertValuesArray) {
        ...
        switch (sUriMatcher.match(uri)) {
            ...
            case IMAGE_URL_QUERY:
                ...
                localSQLiteDatabase.delete(DataProviderContract.PICTURE_URL_TABLE_NAME, null, null);
                ...
                for (int i = 0; i < numImages; i++) {
                    ...
                    localSQLiteDatabase.insert(DataProviderContract.PICTURE_URL_TABLE_NAME,
    ...
    public int update(@NonNull @android.support.annotation.NonNull Uri uri, @Nullable @android.support.annotation.Nullable ContentValues values, @Nullable @android.support.annotation.Nullable String selection, @Nullable @android.support.annotation.Nullable String[] selectionArgs) {
        ...
        switch (sUriMatcher.match(uri)) {
            ...
            case URL_DATE_QUERY:
                ...
                // Create a new writeable database or retrieves a cached one.
                SQLiteDatabase localSQLiteDatabase = mHelper.getWritableDatabase();
